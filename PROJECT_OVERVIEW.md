...
- ✅ `notebooks/3d_focus_stack.ipynb`, `notebooks/3d_pupil_wavefront.ipynb`, `notebooks/3d_resist_depth.ipynb` — validation-first Matplotlib notebooks; focus/resist use 2D physical checks before demo-only 3D views
...
자산 (현재):
  ...
  ✅ 2D validation-first notebooks with demo-only 3D surfaces for focus and resist depth
...
- **2026-04-28** 2D validation-first visualization correction applied.
  - Rebuilt `3d_focus_stack` around a 41-sample x-defocus heatmap and scalar contrast-through-focus curve.
  - Rebuilt `3d_resist_depth` around a 31-sample Beer-Lambert x-depth heatmap plus focus-depth coupled comparison.
  - Reclassified 3D focus/resist surfaces as demo-only views in README and the 3D implementation guide.
